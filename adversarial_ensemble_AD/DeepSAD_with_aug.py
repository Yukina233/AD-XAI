...
data_path = os.path.join(path_project, 'data/banwuli_data/yukina_data/DeepSAD_aug_data')
...
for fault in tqdm(os.listdir(data_path), desc='Total progress'):
    ...
    for i in range(1, int(len(files) + 1)):
        ...
        path_save = os.path.join(path_project, f'adversarial_ensemble_AD/log/banwuli_data/DeepSAD_aug', 'DeepSAD,n_epoch=20',
                                 base_name, f'{i}')
