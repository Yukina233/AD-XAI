...
if __name__ == '__main__':
    ...
    parser.add_argument("--path_train_data", type=str,
                        default=os.path.join(path_project,
                                             f'data/{train_set_name}/yukina_data'))
    parser.add_argument("--path_test", type=str,
                        default=os.path.join(path_project,
                                             f'data/{train_set_name}/yukina_data'))
    parser.add_argument("--dir_model", type=str,
                        default=os.path.join(path_project, f'gaussian_data/models/{train_set_name}/DeepSAD'))
    ...
    param_dir = f'fix_pretrain, net=Dense, std, n_epochs={config.DeepSAD_config["n_epochs"]}, ae_n_epochs={config.DeepSAD_config["ae_n_epochs"]}, lr={config.DeepSAD_config["lr"]}, ae_lr={config.DeepSAD_config["ae_lr"]}'
    ...
    for test_path in tqdm(test_files, desc='Total progress'):
        ...
        for seed in tqdm(range(0, config.seed), desc='Seed progress'):
            ...
            path_save = os.path.join(path_project, f'gaussian_data/log/gaussian/DeepSAD', param_dir,
    ...
    group_results(os.path.join(path_project, f'gaussian_data/log/gaussian/DeepSAD', param_dir))