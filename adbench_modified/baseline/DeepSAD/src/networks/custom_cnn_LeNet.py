...
class SMD_Custom_LeNet(BaseNet):
...
class SMD_Custom_LeNet_Decoder(BaseNet):
...
class SMD_Custom_LeNet_Autoencoder(BaseNet):

    def __init__(self, rep_dim=32):
        super().__init__()

        self.rep_dim = rep_dim
        self.encoder = SMD_Custom_LeNet(rep_dim=rep_dim)
        self.decoder = SMD_Custom_LeNet_Decoder(rep_dim=rep_dim)
...
class GHL_Custom_LeNet(BaseNet):
    ...
    def __init__(self, rep_dim=32):
        ...
        # 假设输入尺寸为 (80)，经过两个核大小为 2 的最大池化层后，输出尺寸将是 (20)
        self.fc1 = nn.Linear(4 * 20, self.rep_dim, bias=False)

    def forward(self, x):
        x = x.view(-1, 1, 80)  # 将输入扩展为 (batch_size, 1, 80)
        x = self.conv1(x)
        x = self.pool(F.leaky_relu(self.bn1(x)))
        x = self.conv2(x)
        x = self.pool(F.leaky_relu(self.bn2(x)))
...
class GHL_Custom_LeNet_Decoder(BaseNet):
    ...
    def __init__(self, rep_dim=32):
        ...
        # Fully connected layer to produce the final output shape
        self.fc2 = nn.Linear(256, 80)  # Adjust the input size as needed
    ...
    def forward(self, x):
        ...
        x = x.view(int(x.size(0)), -1)  # Flatten the feature map
        x = self.fc2(x)  # Fully connected layer to get the output shape (80)
        return x

class GHL_Custom_LeNet_Autoencoder(BaseNet):

    def __init__(self, rep_dim=32):
        super().__init__()

        self.rep_dim = rep_dim
        self.encoder = GHL_Custom_LeNet(rep_dim=rep_dim)
        self.decoder = GHL_Custom_LeNet_Decoder(rep_dim=rep_dim)
