...
if __name__ == '__main__':
    ...
    parser.add_argument("--path_train_data", type=str,
                        default=os.path.join(path_project, f'data/{train_set_name}/yukina_data/ensemble_data, window=100, step=10'))
    parser.add_argument("--dir_model", type=str,
                        default=os.path.join(path_project, f'SMD_dataset/models/{train_set_name}/ensemble'))
    ...
    for experiment_name in os.listdir(path_data_init)[:10]:
        ...
        X_train_init = None
        y_train_init = None
        for train_dataset in os.listdir(experiment_path):
            data = np.load(os.path.join(experiment_path, train_dataset))
        ...
        for iteration in tqdm(range(0, config.n_epochs), desc='Main epochs'):
            if iteration == 0:
                path_train = os.path.join(config.path_train_data, 'init', f'K={config.K}', experiment_name)
            else:
                path_train = os.path.join(config.path_train_data, 'augment', param_dir, experiment_name, f'{iteration - 1}')
            # 遍历所有数据集文件，分别训练各子模型
            for train_dataset in tqdm(os.listdir(path_train), desc='Seperate model train'):
                ...
                dir_model_save = os.path.join(config.dir_model, experiment_name, f'{iteration}')
                ...
                if iteration == 0:
                    ...
                    path_model_save = os.path.join(dir_model_save, f'DeepSAD-{base_name}.pth')
                    # 加载模型
                    path_model_load = os.path.join(config.dir_model, experiment_name, f'{iteration - 1}/DeepSAD-{base_name}.pth')
            ...
            # 将字典存储到文件中
            path_train_result_save = os.path.join(config.path_output, experiment_name, 'loss')
            ...
            # 构造新的训练数据集
            for train_dataset in tqdm(os.listdir(experiment_path), desc='Create new train data'):
                data = np.load(os.path.join(experiment_path, train_dataset))
                X_train = data['X_train']
                y_train = data['y_train']
                ...
                # 保存新的训练数据集
                path_train_new = os.path.join(config.path_train_data, 'augment', param_dir, experiment_name, f'{iteration}')
