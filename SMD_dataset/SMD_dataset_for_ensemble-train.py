...
for dataset_name in tqdm(os.listdir(input_path), desc='Cluster progress'):
    ...
    output_dir_path = os.path.join(output_path, dataset_name.replace('.npz', ''))
    os.makedirs(output_dir_path, exist_ok=True)
    for item in enumerate(groups):
        np.savez(os.path.join(output_dir_path, f'{item[0]}'), X=[0], y=[0], X_train=item[1], y_train=np.zeros(item[1].shape[0]), X_test=X_test,
                 y_test=y_test)
