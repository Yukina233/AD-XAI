...
class hhs():
    def __init__(self):
        self.detect_fault_flag = 0  # 检测故障，0表示检测无故障，1表示检测有故障
        self.real_fault_flag = 0 # 实际故障，0表示检测无故障，1表示检测有故障
        self.Control_limits = 0  # 控制限
        self.real_fault = "qs"   # 真实故障是卡死故障
        self.falsealarm = 0     #虚警率
        self.detect_accuracy = 1       # 故障检测率
        self.amplification_accuracy = 1   # 故障倍幅准确率
        self.detect_time = 0   # 故障检测时间
        self.amplification_time = 53     # 倍幅时间
        self.identification_time = 0     # 辨识时间
        self.estimate_time =0   # 估计时间
        self.positioning_time =0     # 定位时间
        self.fault_degree = 0      # 故障真实程度
        self.diagnose_accuracy =1   # 诊断准确率
        self.error_degree_guess_value = 0  # 故障估计程度
        self.out = []


    def updata(self):
        # if time>49:
        #     self.real_fault_flag = 1
        #     self.Control_limits = 1
        # elif time>49:
        #     self.identification_time = 50.123
        # elif time>49:
        #     self.positioning_time = 50.256
        # elif time>49:
        #     self.estimate_time = 50.347
        # else:
        #     pass

        self.out.append([self.falsealarm,self.detect_accuracy,self.amplification_accuracy,self.detect_time,
                         self.amplification_time,self.identification_time,self.estimate_time,self.positioning_time,
                         self.detect_fault_flag,self.real_fault_flag,self.Control_limits,self.fault_degree,self.diagnose_accuracy,self.error_degree_guess_value])

    def last(self):
        self.out = np.array(self.out)
    def save_npy(self):
        np.save("hhs/out1.npy",np.array(self.out))
...
if __name__ == '__main__':
    ...
    model = load_model(r'..\model\Lstm_Auto_Encoder.h5')  # 自编码器
    model_classify = load_model(r'..\model\Fault_Identif_Train.h5')

    ka_location = load_model(r'..\model\Fault_Location_ks.h5')
    song_location = load_model(r'..\model\Fault_Location_sf.h5')
    que_location = load_model(r'..\model\Fault_Location_rqs.h5')
    ls_location = load_model(r'..\model\Fault_Location_lqs.h5')

    ls1_estimation = load_model(r'origin_model\ls1_estimation.h5')
    ls2_estimation = load_model(r'origin_model\ls2_estimation.h5')
    rqs1_estimation = load_model(r'origin_model\rqs1_estimation.h5')
    rqs2_estimation = load_model(r'origin_model\rqs2_estimation.h5')
    rqs3_estimation = load_model(r'origin_model\rqs3_estimation.h5')
    rqs4_estimation = load_model(r'origin_model\rqs4_estimation.h5')
    th_estimation = load_model(r'origin_model\th_estimation.h5')
    aa = np.zeros([1, 13, 27])
    ...
    A = np.load(r'origin_model\projection.npy')
    train_distance = np.load(r'origin_model\train_distance.npy')
    center = np.load(r'origin_model\center.npy')
    ...
    while True:
        if time.perf_counter() - time1 >= 0.01:
            ...
            print(data)
            if(data[0]>50):
                hhs.real_fault_flag = 1
                hhs.fault_degree = 0.3
            hhs.updata()
            ...
            if apo != []:  # apo为警报
                print('n')
                hhs.detect_fault_flag = 1
                break
            else:
                print('y')
    data = pickle.loads(client_socket.recv(1024))
    hhs.detect_time = data[0]-50
    hhs.detect_accuracy = 0.9946
    CNT = 0
    time1 = time.perf_counter()
    # 积累滑动窗口
    while True:
    ...
    fault_type = model_classify(x_test, training=False)       # 故障辨识
    hhs.identification_time = time.perf_counter()-identification_time    # 故障辨识时间保存
    fault_type_max = np.argmax(fault_type[0])
    print(fault_type[0])
    print(np.array(fault_type[0]).argsort()[::-1])
    ...
    if fault_type_max == 0:
        ...
        positioning_time = time.perf_counter()
        ...
        hhs.positioning_time = time.perf_counter()-positioning_time           # 缺损故障估计时间写入

        estimate_time = time.perf_counter()
        fault_estimation_index = chengdu[np.argmax(fault_estimation[0])]
        hhs.error_degree_guess_value = (2*(np.argmax(fault_estimation[0])+1)-1)*0.1
        hhs.estimate_time = time.perf_counter() - estimate_time             # 故障估计时间
