...
def train_dataset_to_dataframe(dataset):
    images = []
    labels = []
...
def test_dataset_to_dataframe(dataset):
    images = []
    labels = []
    for img, label in dataset:
        if label == 0:
...
test_dataset = datasets.MNIST(root=os.path.join(path_project, 'data/MNIST'), train=False, download=True,
                              transform=transform)
# 转换训练集和测试集
train_df = train_dataset_to_dataframe(train_dataset)
test_df = test_dataset_to_dataframe(test_dataset)
# 保存为CSV文件
