...
def show_samples(dataset_name, param_dir, y, epoch, GAN_config):
    ...
    plt_dir = os.path.join(path_project, f'{name_no_id}_dataset/plots/{str(y)}')
    os.makedirs(plt_dir, exist_ok=True)
    plt.savefig(os.path.join(plt_dir, f'{param_dir}.png'))

if __name__ == '__main__':
    os.environ["CUDA_VISIBLE_DEVICES"] = "3"
    dataset_name = 'MNIST_nonorm_5'
    GAN_config = {
        "seed": 0,
        "latent_dim": 96,
        "lr": 0.0003,
    ...
    y = 5
    epoch = 5
    param_dir = 'GAN_web_w_decay, euc, window=1, step=1, K=7,deepsad_ae_epoch=10,gan_epoch=1,pre_epochs=0,lam1=0,lam2=5,latent_dim=96,lr=0.003,seed=0'

    show_samples(dataset_name, param_dir, y, epoch, GAN_config)