...
class Quantizer(nn.Module):
    def __init__(self, model_config: VQVAEConfig):
        ...
        self.epsilon_prob = model_config.epsilon_prob
        self.register_buffer("embeddings_frequency", torch.ones(self.num_embeddings))
        self.density_factor = model_config.density_factor
    def forward(self, z: torch.Tensor, uses_ddp: bool = False):
        ...
        ) - (self.density_factor * self.embeddings_frequency).to(z.device)

        score = distances - (self.density_factor * torch.log(self.embeddings_frequency)).to(distances.device)

        closest = score.argmin(-1).unsqueeze(-1)
        closest_indices = closest.squeeze()  # 去除多余的维度
        self.embeddings_frequency[closest_indices] += 1
        if self.training and self.epsilon_prob > 0:
            random_matrix = torch.rand(closest.size()[0], device=closest.device) < self.epsilon_prob
            if random_matrix.any():
                # 使用随机选择的方法
                indices = torch.where(random_matrix)[0]
                random_values = torch.randint(0, 256, (indices.numel(),), dtype=torch.int64, device=closest.device)
                closest[indices] = random_values.unsqueeze(-1)
...
class QuantizerEMA(nn.Module):
    ...
    def forward(self, z: torch.Tensor, uses_ddp: bool = False):
        ...
        if not self.training:
            scores = torch.softmax(-distances, dim=1) + (self.density_factor * torch.softmax(self.embeddings_frequency,dim=0))

            closest = scores.argmax(-1).unsqueeze(-1)
        else:
        ...
        if self.training and self.epsilon_prob > 0:
            ...
            chosen = torch.multinomial(probabilities, 1)
            random_matrix = torch.rand(closest.size()[0], device=closest.device) < self.epsilon_prob
            if random_matrix.any():
                # 使用随机选择的方法
                indices = torch.where(random_matrix)[0]
                closest[indices] = chosen[indices]
                # random_values = torch.randint(0, 256, (indices.numel(),), dtype=torch.int64, device=closest.device)
                # closest[indices] = random_values.unsqueeze(-1)
...
class Simplified_QuantizerEMA(nn.Module):
    def     __init__(self, model_config: VQVAEConfig):
