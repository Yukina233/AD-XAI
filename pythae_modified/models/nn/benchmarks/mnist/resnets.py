...
class Encoder_CNN_VAE_MNIST_baseline(BaseEncoder):
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        layers.append(nn.Sequential(nn.Conv2d(self.n_channels, 8, kernel_size=5, stride=2, padding=2),
                                    # nn.BatchNorm2d(8),
                                    # nn.ReLU(),
                                    ))

        layers.append(nn.Sequential(nn.Conv2d(8, 16, kernel_size=5, stride=1, padding=2),
                                    # nn.BatchNorm2d(16),
                                    # nn.ReLU(),
                                    ))

        layers.append(nn.Sequential(nn.Conv2d(16, 32, kernel_size=5, stride=2, padding=2),
                                    # nn.BatchNorm2d(32),
                                    # nn.ReLU(),
                                    ))

        layers.append(nn.Sequential(nn.Conv2d(32, 64, kernel_size=5, stride=1, padding=2),
                                    # nn.BatchNorm2d(64),
                                    # nn.ReLU(),
                                    ))

        layers.append(nn.Sequential(nn.Conv2d(64, 64, kernel_size=5, stride=2, padding=2),
                                    # nn.BatchNorm2d(64),
                                    # nn.ReLU(),
                                    ))
        ...
        self.embedding = nn.Linear(64 * 4 * 4, args.latent_dim)
        self.log_var = nn.Linear(64 * 4 * 4, args.latent_dim)
...
class Decoder_CNN_VAE_MNIST_baseline(BaseDecoder):
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        layers.append(nn.Linear(args.latent_dim, 64 * 7 * 7))

        # layers.append(nn.Sequential(nn.ConvTranspose2d(64, 32, kernel_size=5, stride=2),
        layers.append(nn.Sequential(nn.ConvTranspose2d(64, 32, kernel_size=3, stride=2, padding=1, output_padding=1),
                                    # nn.BatchNorm2d(32),
                                    # nn.ReLU(),
                                    )
                      )
        ...
        layers.append(
            nn.Sequential(
                # nn.ConvTranspose2d(32, 32, kernel_size=5, stride=2, output_padding=1),
        ...
        layers.append(
            nn.Sequential(
                nn.ConvTranspose2d(
                    # 32, 256, kernel_size=5, stride=1, padding=1),
                    32, 256, kernel_size=3, stride=1, padding=1),
                # nn.ReLU(),
                nn.Sigmoid(),
            )
        )
    ...
    def forward(self, z: torch.Tensor, output_layer_levels: List[int] = None):
        ...
        for i in range(max_depth):
            ...
            if i == 0:
                out = out.reshape(z.shape[0], 64, 7, 7)
