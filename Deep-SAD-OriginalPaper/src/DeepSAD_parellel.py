...
def main():
    path_project = '/home/yukina/Missile_Fault_Detection/project/Deep-SAD-OriginalPaper/log/c_not_truncated/'
    seed_num = 3
    config = {
    ...
    experiment_configs = []
    # 10 * 9 * 3 = 270 configs
    if config['normal_class'] == None and config['known_outlier_class'] == None:
        for i in range(0, 10):
            for j in range(0, 10):
                    ...
                    for k in range(0, seed_num):
                        ...
                        config[
                            'xp_path'] = path_project + \
                                         f"{config['dataset_name']}/ae_epochs={config['ae_n_epochs']}/ratio={config['ratio_known_outlier']}/"\
