...
if __name__ == "__main__":
    ...
    # 去除影响力小于0的样本
    negative_influence_df = df[df['influence'] <= -400]
    ...
    model.save(path_project + f'trac/output/model-remove-{percentage}%--400.h5')
    ...
    result.to_csv(path_project + f'trac/output/model-remove-{percentage}%--400-performance.csv', index=False)
    pickle.dump(history, open(path_project + f'trac/output/model-remove-{percentage}%--400-history.pkl', 'wb'))
