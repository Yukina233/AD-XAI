...
sub_path = 'trac/seed=0/increase_2797/'
...
if __name__ == "__main__":
    ...
    repeat_times = 1  # number of times to repeat the increase
    ...
    df = explanation_df.sort_values(by='influence', ascending=False)

    # 增加新的样本
    count_add = int(explanation_df.shape[0] * percentage / 100)
    print(
        f'Number of influence samples add: {count_add * repeat_times}, Percentage: {percentage}%')
    all_sample_id = explanation_df['sample_id'].to_numpy()
    add_sample_id = all_sample_id[:count_add]
    add_sample_X = X_train[add_sample_id]
    add_sample_Y = Y_train[add_sample_id]
    for i in range(repeat_times):
        X_train = np.concatenate((X_train, add_sample_X), axis=0)
        Y_train = np.concatenate((Y_train, add_sample_Y), axis=0)
    ...
    model.save(path_project + sub_path + f'model-add_{percentage}%-repeat_{repeat_times}.h5')
    ...
    result.to_csv(path_project + sub_path + f'model-add_{percentage}%-repeat_{repeat_times}-performance.csv', index=False)
    pickle.dump(history, open(path_project + sub_path + f'model-add_{percentage}%-repeat_{repeat_times}-history.pkl', 'wb'))
