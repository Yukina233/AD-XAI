...
class Encoder_ResNet_VAE_Missile(BaseEncoder):
    """
...
class Decoder_ResNet_AE_Missle(BaseDecoder):
    """
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        layers.append(nn.Linear(args.latent_dim, 256))

        layers.append(nn.ConvTranspose2d(128, 128, 3, 2, padding=1, output_padding=(1, 0)))
        ...
        layers.append(
            nn.Sequential(
                nn.ConvTranspose2d(
                    64, self.n_channels, 4, 2, padding=1, output_padding=(0, 1)
                ),
                nn.Sigmoid(),
            )
        )
    ...
    def forward(self, z: torch.Tensor, output_layer_levels: List[int] = None):
        ...
        for i in range(max_depth):
            ...
            if i == 0:
                out = out.reshape(z.shape[0], 128, 1, 2)
