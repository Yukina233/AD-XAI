...
def run(seed=0, input_path='', path_results='', n_estimators=6, n_ensemble=60):
    ...
    for id, file in enumerate(train_files):
        ...
        clf = DIF(random_state=seed, n_estimators=n_estimators, n_ensemble=n_ensemble)
        clf.fit(data)
...
def run_1_by_1(seed=0, input_path='', path_results='', n_estimators=6, n_ensemble=60):
    train_files = glob(os.path.join(input_path, '*train.csv'))
    test_files = glob(os.path.join(input_path, '*test.csv'))
...
if __name__ == '__main__':
    ...
    path_results = os.path.join(path_project,
                                f'pyod_algorithm/DIF/results/{dataset_name}/{suffix}/n_est={n_estimators}, n_ens={n_ensemble}')
    ...
    for seed in range(3):
        if dataset_name == 'SMD':
            run_1_by_1(seed, path_data, path_results, n_estimators, n_ensemble)
        else:
            run(seed, path_data, path_results, n_estimators, n_ensemble)
