...
def run(seed=0, input_path='', path_results='', epoch_num=10):
    ...
    for id, file in enumerate(train_files):
        ...
        clf = AutoEncoder(random_state=seed, epoch_num=epoch_num)
        clf.fit(data)
...
def run_1_by_1(seed=0, input_path='', path_results='', epoch_num=10):
    train_files = glob(os.path.join(input_path, '*train.csv'))
    test_files = glob(os.path.join(input_path, '*test.csv'))
...
if __name__ == '__main__':
    ...
    path_results = os.path.join(path_project,
                                f'pyod_algorithm/AE/results/{dataset_name}/{suffix}/epoch_num={epoch_num}')
    ...
    for seed in range(3):
        if dataset_name == 'SMD':
            run_1_by_1(seed, path_data, path_results, epoch_num)
        else:
            run(seed, path_data, path_results, epoch_num)
