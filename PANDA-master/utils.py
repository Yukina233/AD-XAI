...
def get_outliers_loader(batch_size):
    dataset = torchvision.datasets.ImageFolder(root=os.path.join(path_project, 'data/tiny'), transform=transform_color)
...
def get_loaders(dataset, label_class, batch_size):
    if dataset in ['cifar10', 'fashion']:
        if dataset == "cifar10":
            ...
            coarse = {}
            trainset = ds(root=os.path.join(path_project, 'Deep-SAD-OriginalPaper/data'), train=True, download=True, transform=transform, **coarse)
            testset = ds(root=os.path.join(path_project, 'Deep-SAD-OriginalPaper/data'), train=False, download=True, transform=transform, **coarse)
